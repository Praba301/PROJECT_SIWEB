...
export default function PengirimanClient({ 
  ...
  const data = dataDariDatabase.map((row, idx) => {
    const statusVariations = [
      { status: "Berlayar", badge: "bg-[#3B82F6]/10 text-[#60A5FA] border border-[#3B82F6]/20" },
      { status: "Dimuat", badge: "bg-[#F59E0B]/10 text-[#FCD34D] border border-[#F59E0B]/20" },
      { status: "Diproses", badge: "bg-[#6B7280]/10 text-[#9CA3AF] border border-[#6B7280]/20" },
      { status: "Terkirim", badge: "bg-[#22C55E]/10 text-[#4ADE80] border border-[#22C55E]/20" },
    ];
    const currentStatus = statusVariations[idx % statusVariations.length];
      ...
      <div className="bg-[#13131F] border border-[#1E1E2E] rounded-2xl p-6 shadow-xl mt-6 animate-slide-up">
              ...
              {data.map((item, idx) => (
                ...
                >
                  ...
                  <div className="flex-1 text-[#A0A0B0] font-medium group-hover:text-white transition-colors duration-300 flex items-center gap-2">
                    <div className="w-6 h-6 rounded-md bg-[#1E1E2E] flex items-center justify-center text-[10px] group-hover:bg-[#A855F7] group-hover:text-white transition-colors duration-300 shadow-sm">
                      🏢
                    </div>
                    {item.pengirim}
                  </div>
                  ...
                  <div className="w-32 flex justify-center">
                    <span className={`px-3 py-1.5 rounded-md text-xs font-bold tracking-wide flex items-center gap-1.5 shadow-[0_0_10px_rgba(0,0,0,0.1)] transition-transform duration-300 group-hover:scale-110 ${item.badge}`}>
        ...
        <div className="flex justify-between items-center mt-8 pt-6 border-t border-[#1E1E2E]">
          <button 
            ...
            className="px-4 py-2 text-sm font-medium text-[#A0A0B0] bg-[#1A1A24] rounded-lg border border-[#1E1E2E] hover:border-[#A855F7]/50 hover:text-white disabled:opacity-40 disabled:hover:border-[#1E1E2E] disabled:cursor-not-allowed transition-all"
          >
            ← Sebelumnya
          </button>
          
          <span className="text-sm text-[#6B6B80]">
            Halaman <span className="text-white font-bold mx-1">{currentPage}</span> dari <span className="text-white font-bold mx-1">{totalPages}</span>
          </span>
